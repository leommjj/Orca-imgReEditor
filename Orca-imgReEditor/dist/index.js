...
(function() {
    ...
    // ---------- 配置 ----------
    // 获取当前脚本的路径，用于构建正确的fabric.min.js路径
        ...
        try {
            ...
            if (error.stack) {
                ...
                for (const line of stackLines) {
                    if (line.includes('http') || line.includes('file:')) {
            ...
            console.warn('通过错误堆栈获取脚本路径失败:', e);
        }
    };
    
    const CONFIG = {
        fabricPath: getCurrentScriptPath() + 'fabric.min.js', // 动态构建fabric.js路径
    ...
    // ---------- 核心：通过DOM获取图片块的原始URL ----------
    function getImageUrlByBlockId(blockId) {
        try {
            // 直接从DOM读取图片URL
            const blockEl = document.querySelector(`[data-block-id="${blockId}"]`);
            if (!blockEl) {
                throw new Error(`未找到ID为${blockId}的块元素`);
            }
            const imgEl = blockEl.querySelector('img');
            if (!imgEl) {
                throw new Error(`ID为${blockId}的块中未找到图片元素`);
            }
            if (!imgEl.src) {
                throw new Error(`图片元素的src属性为空`);
            }
            console.log('📸 从DOM成功获取图片URL:', imgEl.src);
            return imgEl.src;
        } catch (err) {
            console.error('⚠️ DOM获取URL失败：', err);
            return '';
        }
    }
    ...
    async function getImageData(blockId) {
        try {
            // 获取原始图片URL
            const imgUrl = getImageUrlByBlockId(blockId);
            if (!imgUrl) {
    ...
    function addEditButtons() {
        ...
        toolbars.forEach(toolbar => {
            ...
            editBtn.addEventListener('click', async () => {
                ...
                // 方法1: 查找工具栏所在的图像块（最直接的方式）
                if (toolbar) {
                    // 向上查找所有带data-block-id的元素
                    let current = toolbar;
                    const blockElements = [];
                    ...
                    while (current && current !== document.body) {
                        if (current.hasAttribute('data-block-id')) {
                            ...
                            // 如果找到明显的图像块，优先使用
                            if (current.classList.contains('orca-image-block') || 
                                current.querySelector('img') || 
                                current.querySelector('[data-block-type="image"]')) {
                                blockEl = current;
                                break;
                            }
                        }
                    ...
                    // 方法3: 最后的尝试 - 查找整个文档中与工具栏相关的图像块
                    if (!blockEl) {
                        const allBlocks = document.querySelectorAll('[data-block-id]');
                        for (const block of allBlocks) {
                            if (block.contains(toolbar) && block.querySelector('img')) {
                                blockEl = block;
                                break;
                            }
                        }
                ...
                console.log('🔍 块ID查找结果:', {
                    ...
                    hasImage: !!blockEl?.querySelector('img'),
                    blockClass: blockEl?.className
                });
                
                if (!blockEl) {
                    alert('❌ 无法找到图像块，请确保在图像上操作');
                    return;
                }
                
                currentBlockId = blockEl.getAttribute('data-block-id');
                console.log('✅ 最终确定的图像块ID:', currentBlockId);
    ...
    function bindEditorEvents() {
        ...
        editModal.querySelector('.btn-save').onclick = async function() {
            ...
            try {
                ...
                console.log('🔄 准备保存编辑后的图片...');
                console.log('📦 图片数据大小:', Math.round(editedBase64.length / 1024), 'KB');

                // 按路径覆盖保存 - 不使用API工具
                if (window.orca?.invokeBackend) {
                    console.log('🔌 使用虎鲸invokeBackend上传图片...');
                    console.log('🎯 块ID:', currentBlockId);
                    
                    try {
                        // 获取原始图片URL
                        const originalImageUrl = getImageUrlByBlockId(currentBlockId);
                        console.log('🔗 原始图片完整URL:', originalImageUrl);
                        
                        // 从URL中提取文件名
                        let originalFileName = '';
                        
                        if (originalImageUrl) {
                            console.log('🔍 从URL提取原始文件名:', originalImageUrl);
                            
                            // 处理URL
                            let processedUrl = originalImageUrl;
                            ...
                            if (processedUrl.startsWith('file:///')) {
                                ...
                                processedUrl = processedUrl.split('/').slice(3).join('/');
                            }
                            ...
                            // 获取文件名
                            originalFileName = processedUrl.split('/').pop() || '未知文件名';
                            
                            console.log('📁 提取的文件名:', originalFileName);
                        } else {
                            originalFileName = '未知文件名';
                            console.log('📁 使用默认文件名:', originalFileName);
                        }
                        ...
                        console.log('📋 最终确定的文件信息:', {
                            blockId: currentBlockId,
                            originalFileName: originalFileName,
                            originalImageUrl: originalImageUrl
                        });
                        
                        // Base64直接转ArrayBuffer，避免使用Blob
                        function base64ToBuffer(base64) {
                            try {
                                ...
                                throw new Error('Base64转ArrayBuffer失败: ' + err.message);
                            }
                        }
                        
                        // 直接转换为ArrayBuffer
                        const arrayBuffer = base64ToBuffer(editedBase64);
                        console.log('📦 转换为二进制数据，大小:', Math.round(arrayBuffer.byteLength / 1024), 'KB');
                        
                        // 验证文件名
                        if (!originalFileName || originalFileName.trim() === '') {
                            originalFileName = '未知文件名.png';
                            console.warn('⚠️ 文件名无效，使用默认文件名:', originalFileName);
                        }
                        
                        // 使用虎鲸API进行静默保存
                        console.log('📤 使用虎鲸API保存图片，文件名:', originalFileName);
                        
                        // 上传修改后的图片
                        const assetPath = await window.orca.invokeBackend(
                            "upload-asset-binary", 
                            "image/png", 
                            arrayBuffer,
                            originalFileName
                        ...
                        if (!assetPath) {
                            throw new Error('上传返回的资源路径为空');
                        }
                        
                        console.log('✅ 成功上传图片:', assetPath);
                        console.log('📁 上传时使用的文件名:', originalFileName);
                        
                        // 获取当前块数据并更新
                        const currentBlock = await window.orca.invokeBackend("get-block", currentBlockId);
                        const updatedBlock = await window.orca.invokeBackend(
                            "update-block", 
                            currentBlockId,
                            {
                                ...currentBlock,
                        ...
                        console.log('✅ 成功更新图片块:', updatedBlock);
                        
                        // 静默保存，不显示提示
                        console.log('✅ 图片已静默保存成功');
                    ...
                    console.error('❌ 虎鲸API不可用');
                    throw new Error('虎鲸API不可用，请确保插件在虎鲸环境中运行');
                }
